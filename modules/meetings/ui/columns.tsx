...
export const columns: ColumnDef<meetingGetMany[number]>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className=" flex flex-col gap-y-1">
        <span className="font-semibold capitalize tracking-tighter">{row.original.name}</span>
  ...
  },
  {
    ...
    return (
      <Badge 
      variant="outline"
      className={cn("capitalize tracking-tighter [&>svg]:size-4 text-muted-foreground",
    ...
    return (
      <Badge 
      variant="outline"
      className={cn("capitalize tracking-tighter [&>svg]:size-4 flex items-center gap-x-2 ",
        statusIconMap[row.original.status as keyof typeof statusIconMap]
      )}
      >
